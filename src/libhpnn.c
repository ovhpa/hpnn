...
#else /*DEBUD*/
        lib_runtime.cudas.mem_model=CUDA_MEM_EXP;/*the only option for DEBUG*/
#endif /*DEBUG*/
    }
...
#define _K ((kernel_ann *)(_CONF.kernel))
        ...
        case NN_TYPE_ANN:
            ...
            if(cudas->mem_model!=CUDA_MEM_CMM){
                ...
                CUDA_SYNC();/*we are still on GPU[0]*/
                ARRAY_CP(tr_in,_K->in,_K->n_inputs);
