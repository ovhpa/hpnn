...
#define CUDA_G2G_SCP(src,dest,size,type,stream) do{\
    cudaError_t _err;\
    _err=cudaMemcpyAsync(dest,src,(size)*sizeof(type),\
...
#ifndef DEBUG
/* DeviceReset as initialization seems severe... SetDevice instead? */
#define CUDA_INITIALIZE() do{\
    cudaDeviceReset();\
}while(0)
#define CUDA_GET_DEV(ngpu) do{\
    cudaGetDeviceCount(&(ngpu));\
}while(0)
#define CUDA_SET_DEV(cudas,gpu) do{\
    cudaSetDevice(gpu);\
}while(0)
#define CUDA_SYNC() do{\
...
#define CUDA_GET_DEV(ngpu) do{\
    CUresult _err;\
    CUdevice _dev_id;\
    _err=cuDeviceGet(&(_dev_id),0);\
    if(_err!=CUDA_SUCCESS){\
        _OUT(stderr,"GPU can't get DEVICE (function %s, line %i)\n",\
            FUNCTION,__LINE__);\
        _OUT(stderr,"CUDA report: %s\n",cudaGetErrorString(_err));\
        exit(-1);\
    }\
    _err=cuDevicePrimaryCtxRetain(&(lib_runtime.cudas.context[0]),_dev_id);\
    if(_err!=CUDA_SUCCESS){\
        _OUT(stderr,"GPU Primary context ERROR (function %s, line %i)\n",\
            FUNCTION,__LINE__);\
        _OUT(stderr,"CUDA report: %s\n",cudaGetErrorString(_err));\
        exit(-1);\
    }\
    for(int _i=0;_i < N_GPU_CTX;_i++){\
        _err=cuCtxCreate(&(lib_runtime.cudas.context[_i]),0,_dev_id);\
        if(_err!=CUDA_SUCCESS){\
            _OUT(stderr,"GPU fail to create CONTEXT (function %s, line %i)\n",\
                FUNCTION,__LINE__);\
            _OUT(stderr,"CUDA report: %s\n",cudaGetErrorString(_err));\
            exit(-1);\
        }\
...
#define CUDA_SET_DEV(cudas,gpu) do{\
    cuCtxSetCurrent((cudas).context[gpu]);\
}while(0)
#define CUDA_SYNC() do{\
    cuCtxSynchronize();\
}while(0)
...
#define CUDA_FINALIZE() do{\
    ...
    for(int _i=0;_i < N_GPU_CTX;_i++){\
        cuCtxDestroy(lib_runtime.cudas.context[_i]);\
    }\
    cuDevicePrimaryCtxRelease(_dev_id);\
}while(0)
#endif /*DEBUG*/
...
typedef struct {
    UINT n_gpu;
#ifdef DEBUG
    /*we change context to debug multi-GPU*/
    CUcontext context[N_GPU_CTX];
#endif /*DEBUG*/
#ifdef _CUBLAS
